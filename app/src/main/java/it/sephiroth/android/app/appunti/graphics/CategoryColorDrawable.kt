...
class CategoryColorDrawable(context: Context, color: Int) : ColorDrawable(color) {
    ...
    val strokeWidth =
        context.resources.getDimension(R.dimen.appunti_category_color_drawable_stroke_width)
    ...
    override fun onStateChange(stateSet: IntArray?): Boolean {
        ...
        mChecked =
            mChecked or (stateSet?.contains(android.R.attr.state_selected) ?: kotlin.run { false })
    ...
    override fun draw(canvas: Canvas) {
        canvas
            .drawCircle(
                dstBounds.centerX().toFloat(),
                dstBounds.centerY().toFloat(),
                (dstBounds.width() / 2).toFloat(),
                fillPaint
            )
        canvas.drawCircle(
            dstBounds.centerX().toFloat(),
